...
module.exports = function(githubTarget, micropubDocument, siteUrl, options) {
  ...
  if (options.deriveCategory) {
    categoryDeriver = properties =>
      matchPropertiesToConditions(options.deriveCategory, properties);
  }

  return Promise.resolve(
    options.noAutoConfigure
      ? options
      : autoConfigure(publisher).then(autoConfig =>
          Object.assign(autoConfig, options)
        )
  )
    .then(options => {
      ...
      [
        ...
        if (Array.isArray(value)) {
          options[key] = properties =>
            matchPropertiesToConditions(value, properties);
        }
      });
    ...
    .then(formatted => {
      ...
      return Promise.all(
        (formatted.files || []).map(file => {
          return publisher
            .publish(file.filename, file.buffer, {
              force: force,
    ...
    .then(formatted => {
      ...
      // SO MUCH SHAME!
      formatted.content.substring(0, formatted.content.length - 5) +
        `\npermalink:${options.permalinkStyle}\n---\n`;
